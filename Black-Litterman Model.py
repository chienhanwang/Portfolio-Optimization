...
def output(weights): 
    return -(portfolioReturn(weights) - portfolioVolatility(weights))
...
for tret in trets:
    ...
    res = sco.minimize(output, initialWeights, method='SLSQP', bounds=bnds, constraints=cons) #最佳化算法採用SLSQP，原因是限制式為線性不等式。
